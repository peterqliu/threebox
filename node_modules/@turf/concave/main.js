...
function lineDissolve(geojson, options) {
    ...
    // Validation
    if (invariant.getType(geojson) !== 'FeatureCollection') throw new Error('geojson must be a FeatureCollection');
    ...
    var result = [];
    var lastLine = meta.lineReduce(geojson, function (previousLine, currentLine) {
    ...
    else return helpers.multiLineString(result.map(function (line) { return line.coordinates; }));
}
...
function mergeLineStrings(a, b) {
    ...
    return helpers.lineString(coords);
}
...
function polygonDissolve(geojson, options) {
    ...
    var geoms = [];
    meta.flattenEach(geojson, function (feature$$1) {
        geoms.push(feature$$1.geometry);
    });
    var topo = topojsonServer.topology({geoms: helpers.geometryCollection(geoms).geometry});
    return topojsonClient.merge(topo, topo.objects.geoms.geometries);
}
...
function concave(points, options) {
    ...
    if (!helpers.isNumber(maxEdge)) throw new Error('maxEdge is invalid');
    ...
    return helpers.feature(dissolved);
}
...
function removeDuplicates(points) {
    ...
    return helpers.featureCollection(cleaned);
}
